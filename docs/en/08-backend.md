...
| `ws:user:password` | `Console/SetUserPasswordCommand.php` | The way back into an installation nobody can sign into. It **generates** the password by default and prints it once; `--haslo` is the exception, because a password passed as an argument stays in the shell history. It says out loud the two things the operator cannot see: tokens already issued keep working until they expire (D-017), and a deactivated account will not sign in on a password alone. |
