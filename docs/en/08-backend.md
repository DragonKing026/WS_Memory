...
| `Identity/PasswordPolicy.php` | **The one password rule of this installation** — and the one generator that satisfies it. It became a class when a second door appeared: `POST /api/invitations/accept` and `ws:user:password` both set passwords, and a rule written down twice becomes two rules the moment somebody relaxes one of them while believing the other still holds. Twelve characters, no composition rules (length beats character classes, and rules only push people towards `Haslo123!`), and a refusal for anything known from a public breach. |
| `Identity/ResetUserPassword.php` | Sets a new password on an account that exists. **The audit entry names no actor**, and that is the fact it records: nobody is signed in on a console, and naming the account itself would read as "they reset their own password" — the same reasoning as the actorless `space.member_added` entry in `AcceptInvitation`. A deactivated account is **not** refused: the password grants it nothing, so the entry cannot overstate access — unlike granting a role, which is refused for exactly that reason. It does not validate the password; validation lives at the edge, using the same rule. |
...
| `ws:user:password` | `Console/SetUserPasswordCommand.php` | The way back into an installation nobody can sign into. It **generates** the password by default and prints it once; `--password` is the exception, because a password passed as an argument stays in the shell history. It says out loud the two things the operator cannot see: tokens already issued keep working until they expire (D-017), and a deactivated account will not sign in on a password alone. |
...
Four commands take an e-mail address as their first argument, and
